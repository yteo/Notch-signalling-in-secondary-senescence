...
freebayes -f /data/hg19_mVenus_puro_neo.fa -C 1 ${file2}|grep "chr11"|grep -w "534288" > ${file2%.bam}.vcf; done; for file3 in *.vcf; do a=$(awk '$4 == "C" && $5 == "A" {print $0}' ${file3}|cut -f 10|cut -d ":" -f3,5|cut -d ":" -f1); b=$(awk '$4 == "C" && $5 == "A" {print $0}' ${file3}|cut -f 10|cut -d ":" -f3,5|cut -d ":" -f2)
